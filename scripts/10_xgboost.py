"""STAGE ④ — XGBoost
target: binding_affinity / feature: 동일 7개. SOP 4-4 권장 규제(max_depth~3, lr~0.05, reg_alpha/lambda, subsample/colsample 0.8) 적용.
"""
import pandas as pd
import numpy as np
import joblib
...
metrics_df.to_csv(MODEL_DIR / "xgboost_metrics.csv")
...
ax.set_title(f"XGBoost: 실제값 vs 예측값 (test, R²={m_test['R2']:.3f})")
ax.legend()
ax.set_aspect("equal", adjustable="box")
fig.savefig(FIG_DIR / "xgboost_actual_vs_pred.png", dpi=150, bbox_inches="tight")
plt.close(fig)

imp_df = pd.DataFrame({"feature": FEATURES, "importance": model.feature_importances_}).sort_values("importance", ascending=False)
print("\n=== feature importance ===")
print(imp_df.to_string(index=False))
imp_df.to_csv(MODEL_DIR / "xgboost_feature_importance.csv", index=False)

joblib.dump({"model": model, "feature_columns": FEATURES}, MODEL_DIR / "xgboost.pkl")
print("\n저장 완료: xgboost_metrics.csv, xgboost_feature_importance.csv, xgboost.pkl, xgboost_actual_vs_pred.png")
